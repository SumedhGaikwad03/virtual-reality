...
### 2. Rule-Based Property Discovery Assistant Architecture

#### 2.1 Overview & Design Philosophy
The Property Discovery Assistant is a deterministic, button-driven concierge engine designed to guide users through available residential inventory without the latency, ungrounded hallucinations, or security vulnerabilities of a Large Language Model (LLM).

```
[Public Search Catalog Endpoint]
       GET /api/search/catalog (Loaded once on search session initialization)
                       ↓
[In-Memory Browser Catalog Cache] (search-catalog.service.ts)
                       ↓
[Reactive Query Builder Engine] (query-builder.ts & useSearchChat.ts)
   ├── Candidate Filtering (filterCatalog)
   ├── Unique Project Extraction (getUniqueProjects)
   ├── Viable Option Derivation (rule.getOptions)
   ├── No-Op Elimination (Skips single-choice or non-narrowing rules)
   └── Intelligent Stopping (PROJECT_STOPPING_THRESHOLD = 3 unique projects)
                       ↓
[Conversational Presentation Layer]
   ├── SearchAssistant.tsx / PropertyAssistantOverlay.tsx
   ├── ConversationMessages.tsx (✦ Concierge thread)
   ├── QuerySummary.tsx (Subtle removable chips)
   ├── RuleOptions.tsx (Touch-friendly buttons, Change previous answer, Start over)
   ├── SearchAssistantEmptyState.tsx (Targeted filter relaxation)
   └── SearchResults.tsx -> PropertyResultCard.tsx (Deep-link to project?configuration=id)
```

#### 2.2 Public Search Catalog Endpoint (`GET /api/search/catalog`)
The search catalog is a safe, read-only public projection of published inventory generated by `SearchCatalogRepository`.
- **Public Whitelist**:
  - Project: `id`, `name`, `slug`, `status`, `locationName`, `locationSlug`
  - Developer: `id`, `name`, `slug`
  - Configurations: `id`, `bhk`, `carpetArea`, `priceFrom` (stringified paise), `availabilityStatus`
- **Security Boundary**: The catalog never exposes administrative fields, unpublished records, lead records, internal notes, credentials, or private media metadata. The backend remains the sole authority for lead capture and configuration validation.

#### 2.3 Browser-Side In-Memory Filtering vs. Per-Click Requests
- **Single Initial Fetch**: The catalog is fetched once on session startup and cached in browser memory via `search-catalog.service.ts`.
- **Zero Per-Click Latency**: Every button click calculates candidate sets, unique projects, and next viable questions synchronously in $\approx 0\text{ms}$ with zero network round-trips or loading spinners.
- **Resilient Lifecycle**: If catalog loading fails, a clean retry action clears the in-flight cache and re-fetches. If the database catalog is empty, a clear explanation is displayed.

#### 2.4 Grounding, No-Phantom-Options & No-Op Question Elimination
- **Candidate-Grounded Options**: Every option presented (BHKs, locations, developers, budget bands, project statuses, availability) is derived strictly from candidate configurations surviving all active filters.
- **No Phantom Options**: If published catalog inventory contains only 2 and 3 BHK, options for 1, 4, or 5 BHK are never generated or shown.
- **No-Op Elimination**: Rules with $\le 1$ viable choice across the remaining matches (e.g., all remaining projects are in Pune, or all belong to one developer) are skipped automatically.

#### 2.5 Distinct Project-Level Stopping (`PROJECT_STOPPING_THRESHOLD = 3`)
- Stopping counts **distinct projects**, not individual configuration rows. A project with 5 configurations counts as 1 unique project.
- Once user selections narrow candidate inventory to $\le 3$ unique projects (or 1 project), the assistant immediately halts questioning and presents matching project cards.
- Each result card retains its exact configuration deep link (`/:devSlug/:locSlug/:projSlug?configuration=:configId`).

#### 2.6 Targeted Zero-Result Recovery
If a combination of filters yields 0 matches:
- Questioning stops immediately.
- `SearchAssistantEmptyState` generates targeted buttons only for active filters (*"Try all locations"*, *"Adjust budget"*, *"Any configuration"*, *"Any developer"*, *"Any project status"*, *"Any availability"*), plus *"Start over"*.
...
## 6. SEO Pre-Rendering & Edge Rewrite Architecture (Phase 1 & Phase 2)

### 6.1 Edge-to-Backend Proxy Architecture
The platform implements a server-rendered SEO architecture via Vercel Edge Rewrites (`vercel.json`), routing public crawls to Express pre-rendering endpoints while preserving the client-side SPA lifecycle for discovery and administrative tools.
...
Browser / Googlebot Request
   ...
   ├── /search, /admin, /admin/* ──▶ /index.html (React SPA)
   ├── /api/:path* ───────────────▶ Render Backend API (/api/...)
   ├── /sitemap.xml ──────────────▶ Render Backend (/sitemap.xml)
   ├── /robots.txt ───────────────▶ Render Backend (/robots.txt)
   ├── /projects-in-pune ─────────▶ Render Backend (/seo/city-hub)
   ├── /location/:locationSlug ───▶ Render Backend (/seo/location/:locationSlug)
   ├── /:dev/:loc/:proj ──────────▶ Render Backend (/seo/project/:dev/:loc/:proj)
   ├── /:developerSlug ───────────▶ Render Backend (/seo/developer/:developerSlug)
   └── / ─────────────────────────▶ Render Backend (/seo/home)
            │
            ▼
   Express Backend (seo.routes.ts -> seo-renderer.service.ts)
            │
            ├── Database Query via Repository (Strict Publication Boundary: PUBLISHED only)
            ├── Injects Open Graph, Twitter Cards, Canonical URL
            ├── Injects Schema.org JSON-LD Structured Data
            └── Pre-renders Semantic <main> HTML with Crawlable Links
```

### 6.2 Indexable Public URL Taxonomy

| Public URL Pattern | Target Backend Route | Schema.org Type | Key Content |
| :--- | :--- | :--- | :--- |
| `/` | `/seo/home` | `WebSite`, `Organization` | Platform branding, featured developments, developer links |
| `/projects-in-pune` | `/seo/city-hub` | `Place`, `ItemList`, `BreadcrumbList` | Pune city aggregation, 6 published projects, 4 localities |
| `/location/:locationSlug` | `/seo/location/:locationSlug` | `Place`, `ItemList`, `BreadcrumbList` | Locality-specific project inventory, developer attributions |
| `/:developerSlug` | `/seo/developer/:developerSlug` | `Organization`, `BreadcrumbList` | Developer portfolio, active project cards |
| `/:dev/:loc/:proj` | `/seo/project/:dev/:loc/:proj` | `ApartmentComplex`, `Offer`, `BreadcrumbList` | Project details, pricing from, unit specs, amenities |
| `/search` | `/index.html` | Client SPA | Interactive Tara discovery assistant |
| `/sitemap.xml` | `/sitemap.xml` | XML URLSet | Dynamic index of 16 published URLs with `<lastmod>` |
| `/robots.txt` | `/robots.txt` | Text Directives | Public crawl allowance, admin/internal path exclusion |

### 6.3 Security & Anti-Defacement Controls
- **Strict Publication Boundary:** Queries enforce `publishStatus === "PUBLISHED"` on both Project and Developer. Draft projects or developers return HTTP 404 + `<meta name="robots" content="noindex, nofollow" />`.
- **HTML Character Sanitization:** All entity fields (`name`, `description`, `location`, `title`, `altText`) are escaped via `escapeHtml()`.
