...
### Public Discovery & Tara Architecture Refactor (Phases 1, 2 & 3)
  ...
  - **Direct Property Discovery Entry**: When a session begins, Tara directly initiates the deterministic property discovery sequence (*"What kind of home are you looking for?"*), operating exclusively as a residential property discovery advisor for homebuyers.
  ...
  - **Public Global Navigation Streamlining (Phase 62)**: Removed the `Rentals` navigation link from desktop primary navigation and the mobile drawer (`GlobalHeader.tsx`), establishing a clean 3-link primary header (`Home`, `About`, `Privacy Policy`) and `Contact & Advisory` action button. The Seeker Rental Desk (`/rentals`), Owner Property Desk (`/rentals/list-property`), and backend rental services remain 100% active and accessible independently.
...
- **Lead Operations, Ownership & Role-Based Authorization (Steps 1, 2 & 3)**:
  ...
  - **Location V1 — Step 1: Backend Location Snapshot Foundation**:
    - **Data Model**: Extended `Admin` model with nullable location snapshot fields: `lastLatitude Float?`, `lastLongitude Float?`, and `lastLocationAt DateTime?`. Stored directly on `Admin` with no `LocationHistory` table.
    - **Database Migration**: Deployed `20260920150000_add_admin_location_snapshot_fields` and regenerated Prisma Client.
    - **Self-Update API (`POST /api/admin/location`)**: Authenticated admin updates own location snapshot with `{ latitude, longitude }`. Validation ensures finite numbers in range $[-90, 90]$ and $[-180, 180]$, rejects client-supplied identity/timestamp parameters with HTTP 400 (`INVALID_LOCATION_REQUEST`), and records server timestamp `lastLocationAt = new Date()`.
    - **Founder Multi-Admin Read API (`GET /api/admin/locations`)**: Founder-only endpoint (`requireFounderAuthentication`) returning active administrators with their location snapshot data (`id, name, email, role, latitude, longitude, lastLocationAt`), omitting sensitive credentials. Employees receive 403 Forbidden.
  - **Location V1 — Step 3: Founder Employee Locations View**:
    - **Page (`/admin/locations`)**: Founder-only operational workspace (`<FounderRoute>`) displaying active team members, role badges, last reported timestamps, coordinate technical details, and direct external Google Maps links (`https://www.google.com/maps/search/?api=1&query=<lat>,<lng>`).
    - **Navigation**: Added Founder-only `"Locations"` item to desktop More dropdown and mobile drawer under Management (completely hidden for Employees).
    - **Invariants Preserved**: Reused existing `GET /api/admin/locations` without schema changes, maps SDK, polling, or tracking.

- **Unified Admin "All Leads" Read-Layer Projection (Option A)**:
  - Aggregates residential property `Lead` (PROPERTY) and `RentalEnquiry` (RENTAL) records into a unified read model for the `/admin/leads` workspace.
  - Zero database schema alterations, zero migrations, zero duplicate records, and zero table synchronization.
  - Full RBAC preservation: Founder sees all property leads and rental enquiries; Employee sees owned property leads (`ownerId === employee.id`) + all shared rental enquiries.
  - Frontend type selector (`All Leads`, `Property Leads`, `Rental Enquiries`), dynamic status filtering, restrained `PROPERTY` / `RENTAL` pill badges, contextual requirements breakdown, dynamic View routing (`/admin/leads/:id` vs `/admin/rentals/enquiries/:id`), and type-aware deletion dispatching.
  - Dedicated Rental Desk workspaces (`/admin/rentals/enquiries` and `/admin/rentals/available`) remain 100% functional and intact.

- **PWA Update & Asset Cache Lifecycle Fix**:
  - Restored standard Vite content hashing for entry assets (`assets/index-[hash].js`, `assets/index-[hash].css`) to safely align with Vercel immutable asset caching.
  - Bumped Service Worker cache version to `virtual-reality-admin-shell-v6` with automatic deletion of obsolete `virtual-reality-*` caches on activation.
  - Integrated `controllerchange` listener and single-reload guard in `registerServiceWorker.ts` with startup `registration.update()` check, ensuring installed mobile PWAs update automatically upon deployment without manual uninstallation or storage clearing.

---

## Current Status & Next Steps
- **Completed**: Core Backend, Public Pages, Media Architecture, Tara Conversational Discovery Assistant, Admin Portal & PWA, Security Hardening, SEO Pre-Rendering & Edge Rewrites, Public Rental Desk, Admin Rental Operations (Steps 4A-4C), Admin Sales Leads Multi-Token Search, Hierarchical Developer/Project Activation & Public Visibility (Steps 1-3), Tara Refactor Phases 1-3, Navigation Layout Robustness, Automatic Advisory Popup, Public Navigation Streamlining, Contextual Project Enquiry, Standalone Public Enquiry Page, Dedicated Admin Visits Workspace & Full Visit CRUD (`/admin/visits`), Cloudinary Delivery Optimization Phases 1 & 2 (`f_auto,q_auto` + targeted width delivery), Lead Ownership Foundation (Step 1), Lead Authorization & Founder Reassignment (Step 2), Lead Creator Identity Badge (Step 2.1), Visit Authorization & Scoped Employee Access (Step 3), Rental Enquiry Creator Identity (Step 4), Visit Card Creator Tag, Employee/Founder Admin Access Boundary (Step 5), Role-Aware Admin Dashboard, Founder Lead Reassignment UI (Step 7), Final RBAC Audit & Hardening (Step 8), Location V1 Step 1 (Backend Snapshot Foundation), Location V1 Step 2 (Employee Location Permission & First Snapshot), Location V1 Step 3 (Founder Employee Locations View), Unified Admin All Leads Read-Layer Projection, PWA Update & Asset Cache Lifecycle Fix.
- **Branch**: All core platform, rental capabilities, admin hierarchy controls, streamlined public discovery, media delivery optimizations, role-aware workspaces, lead ownership RBAC milestone, employee location snapshot features, unified leads workspace, and PWA automatic update lifecycle verified on `main`.
