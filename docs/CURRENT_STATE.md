...
- **Lead Operations**: Admin Lead Manager exposes project/developer/configuration context, WhatsApp and `tel:` actions, multi-token whitespace search across name, phone, email, message, notes, and related developer/project/configuration names/localities, instant search clear, and a restrained green outer attention halo for `NEW` leads; `IN_PROGRESS` is displayed as “Ongoing” without changing the persisted enum.
